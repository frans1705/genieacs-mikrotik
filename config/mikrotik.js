...
async function getMikrotikConnection() {
    if (!mikrotikConnection) {
        logger.info('No existing Mikrotik connection, attempting to connect...');
        return await connectToMikrotik();
    }

    // Test connection dengan simple command
    try {
        await mikrotikConnection.write('/system/identity/print');
        return mikrotikConnection;
    } catch (error) {
        logger.warn(`Existing Mikrotik connection failed test: ${error.message}, reconnecting...`);
        mikrotikConnection = null;
        return await connectToMikrotik();
    }
}
