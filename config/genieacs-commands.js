...
async function getDeviceById(deviceId) {
    try {
        ...
        let device = devices.find(d => d._id === deviceId);

        if (!device) {
            // Try URL encoded version
            const encodedId = encodeURIComponent(deviceId);
            device = devices.find(d => d._id === encodedId);
        }

        if (!device) {
            // Try URL decoded version
            const decodedId = decodeURIComponent(deviceId);
            device = devices.find(d => d._id === decodedId);
        }

        if (!device) {
            // Try partial match
            device = devices.find(d => d._id.includes(deviceId) || deviceId.includes(d._id));
        }

        if (!device) {
            // Try searching for devices that contain parts of the ID
            const idParts = deviceId.split('-');
            for (const part of idParts) {
                if (part.length > 3) { // Only search for meaningful parts
                    const partialDevice = devices.find(d => d._id.includes(part));
                    if (partialDevice) {
        ...
        logger.error(`Error getting device by ID ${deviceId}: ${error.message}`);
        return null;
    }
}

async function restartDevice(phoneNumber) {
    try {
        const device = await getDeviceByNumber(phoneNumber);
        if (!device) {
            throw new Error('Device not found');
        }
        return await genieacsApi.reboot(device._id);
    } catch (error) {
        logger.error(`Error restarting device for ${phoneNumber}: ${error.message}`);
        throw error;
    }
}

async function editSSID(phoneNumber, newSSID) {
    try {
        const device = await getDeviceByNumber(phoneNumber);
        if (!device) {
            throw new Error('Device not found');
        ...
        // Use existing GenieACS API to change SSID - only 2.4G and 5G
        return await genieacsApi.setParameterValues(device._id, {
            'InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.SSID': newSSID,
            'InternetGatewayDevice.LANDevice.1.WLANConfiguration.5.SSID': `${newSSID}-5G`
        });
    } catch (error) {
        logger.error(`Error changing SSID for ${phoneNumber}: ${error.message}`);
        throw error;
    }
}

async function editPassword(phoneNumber, newPassword) {
    try {
        const device = await getDeviceByNumber(phoneNumber);
        if (!device) {
            throw new Error('Device not found');
        ...
        // Use existing GenieACS API to change password - cover all possible paths
        return await genieacsApi.setParameterValues(device._id, {
            // 2.4GHz band - multiple possible paths
            ...
            // 5GHz band - multiple possible paths
            'InternetGatewayDevice.LANDevice.1.WLANConfiguration.5.PreSharedKey.1.KeyPassphrase': newPassword,
            'InternetGatewayDevice.LANDevice.1.WLANConfiguration.5.KeyPassphrase': newPassword,
            'InternetGatewayDevice.LANDevice.1.WLANConfiguration.5.PreSharedKey.1.PreSharedKey': newPassword
        });
    } catch (error) {
        logger.error(`Error changing password for ${phoneNumber}: ${error.message}`);
        throw error;
    }
}
...
async function factoryResetDevice(deviceId) {
    try {
        // For device ID, we need to find the device first to get phone number
        const devices = await genieacsApi.getDevices();
        ...
        return await genieacsApi.factoryReset(deviceId);
    } catch (error) {
        logger.error(`Error factory reset device ${deviceId}: ${error.message}`);
        throw error;
    }
}
