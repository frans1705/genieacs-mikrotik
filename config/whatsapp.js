...
function formatWithHeaderFooter(message) {
    ...
    // Format pesan dengan header, separator, dan footer
...
function updateConfig(newConfig) {
    try {
        // Update WhatsApp header jika ada
        if (newConfig.whatsapp_header !== undefined) {
            global.appSettings.whatsapp_header = newConfig.whatsapp_header;
            console.log('WhatsApp header updated:', newConfig.whatsapp_header);
        }
        
        // Update WhatsApp footer jika ada
        if (newConfig.whatsapp_footer !== undefined) {
            global.appSettings.whatsapp_footer = newConfig.whatsapp_footer;
            console.log('WhatsApp footer updated:', newConfig.whatsapp_footer);
        }
        
        // Update WhatsApp separator jika ada
        if (newConfig.whatsapp_separator !== undefined) {
            global.appSettings.whatsapp_separator = newConfig.whatsapp_separator;
            console.log('WhatsApp separator updated:', newConfig.whatsapp_separator);
        }
...
▸ *vcr [username] [profile] [nomor_wa]* — Buat voucher + kirim notif
...
▸ *setdefaultpass [password_baru]* — Ganti password default customer
   Contoh: setdefaultpass 123456
▸ *setadminuser [username_baru]* — Ganti username admin web
   Contoh: setadminuser administrator
▸ *setadminpass [password_baru]* — Ganti password admin web
   Contoh: setadminpass newpassword123
▸ *toggleotp* — Toggle OTP customer ON/OFF
   Contoh: toggleotp
▸ *setotptime [menit]* — Ganti waktu kedaluwarsa OTP
   Contoh: setotptime 5
▸ *settechnician [nomor1,nomor2,...]* — Ganti daftar teknisi
   Contoh: settechnician 6281234567890,6289876543210
▸ *setgenieacs [url] [username] [password]* — Ganti konfigurasi GenieACS
...
async function findDeviceByTag(tag) {
    try {
        ...
        const url = `${baseUrl}/devices/?query=${encodedQuery}`;
        ...
        if (response.data && response.data.length > 0) {
            console.log(`Found device with tag ${tag}:`, response.data[0]._id);
            return response.data[0];
        }
        
        console.log(`No device found with tag: ${tag}`);
        return null;
    } catch (error) {
...
Mohon tunggu sebentar.${FOOTER_INFO}`
        ...
        // Dapatkan konfigurasi dari settings.json
        const genieacsUrl = global.appSettings?.genieacsUrl || process.env.GENIEACS_URL;
        ...
        // Update SSID 5GHz hanya di index 5
        let wifi5GFound = false; // DEKLARASI VARIABEL
        try {
            console.log(`Updating 5GHz SSID using config index 5`);
            await axios.post(
                `${genieacsUrl}/devices/${encodedDeviceId}/tasks`,
                {
                    name: "setParameterValues",
            ...
            console.log(`Successfully updated 5GHz SSID using config index 5`);
            wifi5GFound = true; // SET KE TRUE JIKA BERHASIL
        } catch (error) {
            console.error(`Error updating 5GHz SSID with index 5:`, error.message);
            wifi5GFound = false; // SET KE FALSE JIKA GAGAL
        }
...
_Perubahan selesai pada: ${new Date().toLocaleString()}_${FOOTER_INFO}`;
        ...
        if (customerNumber.match(/^\d+$/) && customerNumber.length >= 10) {
            try {
                ...
                // Buat pesan notifikasi untuk pelanggan dengan format dari settings
                const notificationMessage = `📢 *INFORMASI PERUBAHAN PASSWORD WIFI*
...
Perubahan akan diterapkan dalam beberapa menit.`;
                ...
                // Kirim pesan menggunakan sock dengan format dari settings
                await sock.sendMessage(`${formattedNumber}@s.whatsapp.net`, { 
                    text: formatWithHeaderFooter(notificationMessage)
                });
        ...
        console.error('Error in handleAdminEditPassword:', error);
        await sock.sendMessage(adminJid, { 
...
async function handleAdminEditSSID(remoteJid, params) {
    try {
        ...
        // Dapatkan SSID saat ini untuk perbandingan
        const currentSSID = getSSIDValue(device, '1') || 'N/A';
...
async function changePassword(deviceId, newPassword) {
    try {
        ...
        // Perbarui parameter untuk 2.4GHz WiFi - cover all possible paths
        ...
        // Perbarui parameter untuk 5GHz WiFi - cover all possible paths
        const updatePass5Task = {
            ...
            parameterValues: [
                ...
                ["InternetGatewayDevice.LANDevice.1.WLANConfiguration.5.PreSharedKey.1.PreSharedKey", newPassword, "xsd:string"]
            ]
        };
...
async function handleAdminEditPassword(remoteJid, customerNumber, newPassword) {
    try {
        ...
        // Buat task untuk mengubah password 2.4GHz - cover all possible paths
        const updatePass24Task = {
            name: "setParameterValues",
            parameterValues: [
                ["InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.PreSharedKey.1.KeyPassphrase", newPassword, "xsd:string"],
                ...
                try {
                    ...
                    const updatePassAltTask = {
                        ...
                        parameterValues: [
                            ...
                            [`InternetGatewayDevice.LANDevice.1.WLANConfiguration.${idx}.KeyPassphrase`, newPassword, "xsd:string"],
                            [`InternetGatewayDevice.LANDevice.1.WLANConfiguration.${idx}.PreSharedKey.1.PreSharedKey`, newPassword, "xsd:string"]
                        ]
                    };
                ...
                try {
                    ...
                    const updatePass2Task = {
                        ...
                        parameterValues: [
                            ...
                            ["InternetGatewayDevice.LANDevice.1.WLANConfiguration.2.PreSharedKey.1.PreSharedKey", newPassword, "xsd:string"]
                        ]
                    };
...
async function changeSSID(deviceId, newSSID) {
    try {
        ...
        console.error(`Error changing SSID: ${error.message}`);
        return { success: false, message: `Gagal mengubah SSID: ${error.message}` };
    }
}
...
async function getAllDevices() {
    try {
        ...
        console.error('Error getting all devices:', error.message);
        return [];
    }
}
...
async function handleResourceInfo(remoteJid) {
    ...
    try {
        ...
        // Ambil informasi resource dengan konfigurasi terbaru
...
return;
        ...
        // Handler set customer default password
        if (command.startsWith('setdefaultpass ') || command.startsWith('setdefaultpassword ')) {
            if (!isAdmin) {
                await sendFormattedMessage(remoteJid, '❌ *Hanya admin yang dapat mengubah customer default password!*');
                return;
            }
            const newPassword = messageText.split(' ').slice(1).join(' ').trim();
            if (!newPassword) {
                await sendFormattedMessage(remoteJid,
                    '❌ *Format salah!*\n\n' +
                    'Format yang benar:\n' +
                    'setdefaultpass [password_baru]\n\n' +
                    'Contoh:\n' +
                    'setdefaultpass 123456\n' +
                    'setdefaultpassword newpass123'
                );
                return;
            }
            ...
            if (newPassword.length < 4) {
                await sendFormattedMessage(remoteJid, '❌ *Password minimal 4 karakter!*');
                return;
            }

            try {
                let settings = getAppSettings();
                const oldPassword = settings.customer_default_password || '1234';
                settings.customer_default_password = newPassword;
                fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2));

                await sendFormattedMessage(remoteJid,
                    `✅ *Customer Default Password berhasil diubah!*\n\n` +
                    `Password lama: ${oldPassword}\n` +
                    `Password baru: ${newPassword}\n\n` +
                    `ℹ️ Password ini akan digunakan sebagai default untuk customer baru di web interface.`
                );
            } catch (error) {
                console.error('Error updating customer default password:', error);
                await sendFormattedMessage(remoteJid,
                    '❌ *Terjadi kesalahan saat mengubah password!*\n\n' +
                    'Silakan coba lagi nanti.'
                );
        ...
        // Handler set admin username
        if (command.startsWith('setadminuser ') || command.startsWith('setadminusername ')) {
            if (!isAdmin) {
                await sendFormattedMessage(remoteJid, '❌ *Hanya admin yang dapat mengubah admin username!*');
                return;
            }
            const newUsername = messageText.split(' ').slice(1).join(' ').trim();
            if (!newUsername) {
                await sendFormattedMessage(remoteJid,
                    '❌ *Format salah!*\n\n' +
                    'Format yang benar:\n' +
                    'setadminuser [username_baru]\n\n' +
                    'Contoh:\n' +
            ...
            if (newUsername.length < 3) {
                await sendFormattedMessage(remoteJid, '❌ *Username minimal 3 karakter!*');
                return;
            }

            if (!/^[a-zA-Z0-9_]+$/.test(newUsername)) {
                await sendFormattedMessage(remoteJid, '❌ *Username hanya boleh mengandung huruf, angka, dan underscore!*');
                return;
            }

            try {
                let settings = getAppSettings();
                const oldUsername = settings.admin_username || 'admin';
                settings.admin_username = newUsername;
                fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2));

                await sendFormattedMessage(remoteJid,
                    `✅ *Admin Username berhasil diubah!*\n\n` +
                    `Username lama: ${oldUsername}\n` +
                    `Username baru: ${newUsername}\n\n` +
                    `ℹ️ Username ini digunakan untuk login ke web interface admin.`
                );
            } catch (error) {
        ...
        // Handler set admin password
        if (command.startsWith('setadminpass ') || command.startsWith('setadminpassword ')) {
            if (!isAdmin) {
                await sendFormattedMessage(remoteJid, '❌ *Hanya admin yang dapat mengubah admin password!*');
                return;
            }
            const newPassword = messageText.split(' ').slice(1).join(' ').trim();
            if (!newPassword) {
                await sendFormattedMessage(remoteJid,
                    '❌ *Format salah!*\n\n' +
                    'Format yang benar:\n' +
                    'setadminpass [password_baru]\n\n' +
                    'Contoh:\n' +
                    'setadminpass newpassword123\n' +
                    'setadminpassword secretpass'
                );
                return;
            }
            ...
            try {
                let settings = getAppSettings();
                const oldPassword = settings.admin_password || 'admin';
                settings.admin_password = newPassword;
                fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2));

                await sendFormattedMessage(remoteJid,
                    `✅ *Admin Password berhasil diubah!*\n\n` +
                    `Password lama: ${oldPassword}\n` +
                    `Password baru: ${newPassword}\n\n` +
                    `ℹ️ Password ini digunakan untuk login ke web interface admin.`
                );
            } catch (error) {
        ...
        // Handler toggle OTP
        if (command === 'toggleotp' || command === 'otptoggle') {
            if (!isAdmin) {
                await sendFormattedMessage(remoteJid, '❌ *Hanya admin yang dapat mengubah pengaturan OTP!*');
                return;
            }

            try {
                let settings = getAppSettings();
                const currentStatus = settings.customer_otp_enabled || false;
                settings.customer_otp_enabled = !currentStatus;
                fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2));

                const statusText = settings.customer_otp_enabled ? 'AKTIF' : 'NONAKTIF';
                const statusIcon = settings.customer_otp_enabled ? '🔐' : '🔓';

                await sendFormattedMessage(remoteJid,
                    `✅ *OTP Customer berhasil diubah!*\n\n` +
                    `${statusIcon} Status OTP: *${statusText}*\n\n` +
                    `${settings.customer_otp_enabled ?
                        'ℹ️ Customer akan menerima kode OTP via WhatsApp saat login ke web interface.' :
                        'ℹ️ Customer dapat login langsung tanpa OTP ke web interface.'
                    }`
                );
            } catch (error) {
                console.error('Error toggling OTP:', error);
                await sendFormattedMessage(remoteJid,
                    '❌ *Terjadi kesalahan saat mengubah pengaturan OTP!*\n\n' +
                    'Silakan coba lagi nanti.'
                );
            }
            return;
        ...
        // Handler set OTP settings
        if (command.startsWith('setotptime ') || command.startsWith('setotpexpiry ')) {
            if (!isAdmin) {
                await sendFormattedMessage(remoteJid, '❌ *Hanya admin yang dapat mengubah pengaturan OTP!*');
                return;
            }
            const minutes = parseInt(messageText.split(' ')[1]);
            ...
            try {
                let settings = getAppSettings();
                const oldTime = settings.otp_expiry_minutes || 5;
                settings.otp_expiry_minutes = minutes;
                fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2));

                await sendFormattedMessage(remoteJid,
                    `✅ *Waktu kedaluwarsa OTP berhasil diubah!*\n\n` +
                    `⏰ Waktu lama: ${oldTime} menit\n` +
                    `⏰ Waktu baru: ${minutes} menit\n\n` +
                    `ℹ️ Kode OTP akan kedaluwarsa setelah ${minutes} menit.`
                );
            } catch (error) {
...
Pesan GenieACS telah diaktifkan kembali.`);
        ...
        if (isAdmin) {
            ...
            // Perintah vcr (voucher dengan notifikasi)
            if (command.startsWith('vcr ')) {
                const parts = messageText.split(' ');
                if (parts.length !== 4) {
                    await sendFormattedMessage(remoteJid, '❌ *Format salah!*\n\n📝 *Format:* `vcr [username] [profile] [nomor_whatsapp]`\n\n📋 *Contoh:* `vcr user1 3k 081321960222`\n\n💡 *Keterangan:*\n• Username dan password akan sama\n• Profile sesuai yang tersedia di MikroTik\n• Nomor WhatsApp untuk kirim notifikasi');
                    return;
                }
                ...
                if (!phoneNumber.match(/^(08|628)\d{8,12}$/)) {
                    await sendFormattedMessage(remoteJid, '❌ *Format nomor WhatsApp salah!*\n\n📱 *Format yang benar:*\n• 081234567890\n• 628123456789');
                    return;
                }

                // Normalize nomor ke format 628xxx
                let normalizedNumber = phoneNumber;
                if (phoneNumber.startsWith('08')) {
                    normalizedNumber = '628' + phoneNumber.substring(2);
                }

                await sendFormattedMessage(remoteJid, `⏳ *Membuat voucher hotspot...*\n\n👤 Username: ${username}\n🔑 Password: ${username}\n📦 Profile: ${profile}\n📱 Kirim ke: ${normalizedNumber}`);

                try {
                    // Import MikroTik functions
                    const { addHotspotUser } = require('./mikrotik');

                    // Buat user hotspot di MikroTik
                    const result = await addHotspotUser(username, username, profile);

                    if (result && result.success) {
                        // Kirim notifikasi ke nomor yang ditentukan
                        try {
                            const voucherMessage = `🎫 *VOUCHER HOTSPOT*\n\n👤 *Username:* \`${username}\`\n🔑 *Password:* \`${username}\`\n📦 *Profile:* ${profile}\n\n📝 *Cara Penggunaan:*\n1. Hubungkan ke WiFi Hotspot\n2. Buka browser\n3. Masukkan username & password\n4. Klik login untuk mulai browsing\n\n⏰ Voucher berlaku sesuai profile yang dipilih\n📞 Hubungi admin jika ada kendala`;

                            const customerChatId = normalizedNumber + '@c.us';
                            await sock.sendMessage(customerChatId, { text: voucherMessage });

                            // Konfirmasi ke admin
                            await sendFormattedMessage(remoteJid, `✅ *Voucher berhasil dibuat!*\n\n👤 Username: \`${username}\`\n🔑 Password: \`${username}\`\n📦 Profile: ${profile}\n📱 Notifikasi terkirim ke: ${normalizedNumber}\n\n💡 Customer sudah menerima detail voucher via WhatsApp`);

                        } catch (whatsappError) {
                            console.error(`Failed to send voucher notification: ${whatsappError.message}`);
                            await sendFormattedMessage(remoteJid, `✅ *Voucher berhasil dibuat!*\n\n👤 Username: \`${username}\`\n🔑 Password: \`${username}\`\n📦 Profile: ${profile}\n\n⚠️ *Voucher dibuat tapi gagal kirim notifikasi ke ${normalizedNumber}*\nSilakan kirim manual detail voucher ke customer`);
                        }
                    } else {
                        await sendFormattedMessage(remoteJid, `❌ *Gagal membuat voucher!*\n\n🔍 *Error:* ${result.message || 'Unknown error'}\n\n💡 *Kemungkinan penyebab:*\n• Username sudah ada\n• Profile tidak ditemukan\n• Koneksi MikroTik bermasalah`);
                    }
                } catch (error) {
                    console.error(`VCR command error: ${error.message}`);
...
async function getDeviceByNumber(phoneNumber) {
    try {
        ...
        // Buat query untuk mencari perangkat berdasarkan tag nomor telepon
        const query = JSON.stringify({
            '_tags': phoneNumber
        });
        ...
        console.error(`Error getting device by number ${phoneNumber}:`, error.message);
        return null;
    }
}